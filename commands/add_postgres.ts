...
const command = new Command()
  ...
  .action(async () => {
    ...
    if (await FileSystem.fileExists("./.env/development/database")) {
      const databaseDevEnv = new TextDecoder().decode(await Deno.readFile("./.env/development/database"));
      ...
      if (contentCheck != 0) {
        console.log("Not continuing as .env/development/database already contained configuration values.")
        Deno.exit(6);
      }
    }

    const fileContents = encoder.encode("POSTGRES_USER=deno\nPOSTGRES_PASSWORD=password\nPOSTGRES_DB=deno");
    await Deno.writeFile("./.env/development/database", fileContents);

    dockerCompose.services.web.env_file = [...dockerCompose.services.web.env_file, ".env/development/database"];
    ...
    await Deno.writeFile("./docker-compose.yml", encoder.encode(newDockerCompose));
